...
def get_binary_solution(df_xsol):
    ...
    for i in range(len(df_xsol)):
        ...
        current_atom = df_xsol.iloc[i]
        antecessors = df_xsol.iloc[i - 3 : i]
        # check if it is a copy
        if current_atom["atom_number"] == antecessors.iloc[0]["atom_number"]:
            # same plane of the antecessors
            b_values.append(0)
        else:            
...
def create_slices(fnames):
        ...
        for this_type in constraint_types:
            ...
            while slice_start + step < n_df:
                ...
                if df.iloc[slice_start + step]["atom_name"] != current_end:
                    raise Exception(
                        f"Slice Error in {fn_bsol}: the slice does not end in {current_end} !"
                    )
    ...
    for this_type in constraint_types:
        B[this_type], count[this_type] = np.unique(
            B[this_type], return_counts=True
        )  # it also converts lists to np.array
    ...
    df = pd.DataFrame(
        {"bsol": b_column, "count": count_column, "constraint": constraint_column}
    )
