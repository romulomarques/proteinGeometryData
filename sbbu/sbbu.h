...
public:
    ...
    void create_planes(int j, int *d, int n)
    {
        ...
        vec3_set(m_y, m_x[3 * j + 0], m_x[3 * j + 1], m_x[3 * j + 2]);
    ...
    // n = len(f)
    inline void reflect(bool *f, int n)
    {
        const int j = m_j;
        // reset x[j,:]
        m_x[3 * j + 0] = m_y[0];
    ...
    // n = len(f)
    inline void reflect_all(int *d, bool *f, int n)
    {
        // reset x[j,:]
        m_x[3 * m_j + 0] = m_y[0];
        m_x[3 * m_j + 1] = m_y[1];
...
public:
...
#ifdef DEBUG
        if (fabs(vec3_dist2(&m_x[3 * i], A) - a2) > m_dtol ||
            fabs(vec3_dist2(&m_x[3 * i], B) - b2) > m_dtol ||
            fabs(vec3_dist2(&m_x[3 * i], C) - c2) > m_dtol)
    ...
    void solve(const edge_t &edge)
    {
        ...
        {
            // TODO it can be placed outside this 'for'
            const double eij = fabs(vec3_dist(xi, xj) - edge.m_l);
            ...
            if (k == kmax) // backtrack
            {
                ...
                ++k;
            cr.reflect(m_f, m_n); // updates x
            ++niters;
        }
        cr.reflect_all(m_d, m_fopt, m_n);
        if (emin > m_dtol) // edges of range 4 allways have two solutions
        {
            char msg[256];
            snprintf(msg, sizeof(msg), "The edge (%d, %d, %f) could not be solved (emin=%g, len(m_d)=%d).\n",
                     edge.m_i + 1, edge.m_j + 1, edge.m_l, emin, m_n);
            throw std::runtime_error(msg);
        }
#ifdef DEBUG
        const double abs_cond = fabs(emin_last - emin);
        if ((nsols > 1) && (edge.m_j - edge.m_i > 4))
            printf("warning: edge(%5d, %5d, %5.4f) is ill conditioned (abs_cond=%3.2E, nsols=%d)\n",
                   edge.m_i + 1, edge.m_j + 1, edge.m_l, abs_cond, nsols);
#endif
    }
